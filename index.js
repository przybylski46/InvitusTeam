...
function generarEmbedReseñas(username",
"promedio",
"total",
"listaReseñas)"{
   "return"{
...
${listaReseñas}`",
      "color":16758784,
      "image":{
         "url":"https://cdn.discordapp.com/attachments/1498040372323024906/1498040507031486474/1000073586.png?ex=69efb671&is=69ee64f1&hm=5d633196a401b97c7429e1bc3c5da5dc553eb0297e03b170e2e374d000e9581c&"
      }
   }";"
}"interactionCreate",
"async interaction =>"{
...
try"{
    ...
    fs.readFileSync(""`./Embeds/${nombre}.json`",
         "utf8"")  
  );  
...
let existente = await Review.findOne("{
         "userId":"persona"
      }");
...
if (existente)"{
      ...
      }"const embedData = generarEmbedReseñas(user.username",
      "0.0",
      "0",
      "Sin reseñas aún"");
...
return interaction.reply("{
  ...
  try"{
    ...
    let perfil = await Review.findOne("{
            "userId":"persona"
         }");
...
if (estrellas < 1 || estrellas > 5){
         "return interaction.reply("{
            "content":"Pon entre 1 y 5 estrellas",
            "ephemeral":true
         }");"
      }{
         "userId":"persona"
...
const promedio = (reviews.reduce((acc",
      "r) => acc + r.estrellas",
      0) / total).toFixed(1);  

const ultimas = reviews.slice(-10).map(r =>"`> <a:Star:1497749189096898680> **${r.estrellas} ▸ ${r.name}:** ${r.comentario}`"").join(""\n"");  
